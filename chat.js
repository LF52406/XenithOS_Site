...
chatFileInput.addEventListener('change', (e) => {
    ...
    if (file.size > 100 * 1024 * 1024) {
        alert('Размер файла превышает лимит в 100 МБ. Пожалуйста, выберите файл поменьше.');
        chatFileInput.value = '';
        return;
    }
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                try {
                    ...
                    uploadModal.classList.remove('active');
                    chatFileInput.value = '';
                    alert('Не удалось обработать ссылку на файл.');
                }
            } else {
                uploadModal.classList.remove('active');
                chatFileInput.value = '';
                alert('Сервер файлов отклонил загрузку.');
            }
        };
